...
router.get('/', (req, res) => {
  try {
    ...
    // schema: documents.category (no doc_type column)
    // v.170: was doc_type — canonical column is category
    const expiringDocs = safeGet(() => db.prepare(`
      SELECT id, title, category AS doc_type, expiry_date
      FROM documents
...
router.get('/attention', (req, res) => {
  try {
    ...
    // schema: hsa_payments.{receipt_location, you_paid} (no receipt_path or amount)
    // v.170: was receipt_path/amount — canonical: receipt_location/you_paid
    try {
      const missingHsa = db.prepare(`
        SELECT COUNT(*) AS n FROM hsa_payments WHERE receipt_location IS NULL AND you_paid > 0
      `).get()?.n || 0;
    ...
    // schema: table is `career_certifications` (no `certifications` table)
    // v.170: was `certifications` — canonical table is `career_certifications`
    try {
      const expiredCerts = db.prepare(`
        SELECT id, name, expiry_date FROM career_certifications
...
router.get('/focus', (req, res) => {
  try {
    ...
    // schema: career_certifications.name (no cert_name column)
    // v.170: was cert_name — canonical column is name
    try {
      db.prepare(`
        SELECT id, name, expiry_date FROM career_certifications
      ...
      `).all().forEach(r => items.push({
        module: 'career', icon: '🎖',
        label: `Cert: ${r.name} expires ${r.expiry_date}`,
        severity: 'warning',
        href: '/career.html',
      }));
    } catch {}
