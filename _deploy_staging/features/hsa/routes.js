...
router.get('/inbox', (req, res) => {
  try {
    ...
    // HSA drafts
    // v202604.143 fix: hsa_payments has `you_paid`, not `amount`.
    // v.170: attachments columns — was a.file_name / a.file_path; canonical: a.original_filename / a.stored_path
    const hsaDrafts = db.prepare(`
      SELECT p.id, p.date, p.you_paid, p.patient, p.provider,
             p.inbox_attachment_id, p.created_at,
             a.original_filename AS file_name, a.stored_path AS file_path, a.mime_type, a.id AS attachment_id
      FROM hsa_payments p
    ...
    // FSA drafts — fsa_payments has `amount` (DOES exist there per mig 118)
    // schema: attachments.original_filename / .stored_path
...
router.post('/inbox/upload', inboxUpload.array('files', 20), (req, res) => {
  try {
    ...
    for (const f of req.files) {
      ...
      // schema: attachments.stored_path (v.170 — was file_path)
      const dup = db.prepare(`
        SELECT id FROM attachments WHERE stored_path LIKE ? LIMIT 1
      ...
      // schema: attachments.{module, original_filename, stored_filename, stored_path, file_size, mime_type, label}
      ...
      `).run(f.originalname, f.originalname, newPath, f.size, f.mimetype);
...
router.delete('/fsa/payments/:id', (req, res) => {
  try {
    ...
    // schema: attachments.stored_path (v.170 — was file_path)
    const row = db.prepare(`
      SELECT p.inbox_attachment_id, a.stored_path AS file_path
