...
async function importAsAttachment(filePath, rule) {
  ...
  // v.170: was attachment_type/file_name/file_path
  ...
  `).run(fname, fname, newPath, stat.size, mime);
  ...
  if (rule.target_table === 'hsa_payments') {
    // schema: hsa_payments.{date, you_paid, status, inbox_attachment_id, hsa_eligible}
    // v.170: was `amount` — canonical column is `you_paid`
    const r = db.prepare(`
      INSERT INTO hsa_payments (date, you_paid, status, inbox_attachment_id, hsa_eligible)
