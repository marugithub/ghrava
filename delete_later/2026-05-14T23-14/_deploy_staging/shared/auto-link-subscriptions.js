...
function loadActiveSubscriptions() {
  ...
  try {
    // schema: subscriptions.{id, name, cost, billing_cycle, status} (mig 109b)
    // v.170: was monthly_amount/billing_frequency/is_active — canonical cols are cost/billing_cycle/status
    const rows = db.prepare(`
      SELECT id, name, cost, billing_cycle
      FROM subscriptions
      WHERE COALESCE(status,'active') = 'active' AND cost IS NOT NULL AND cost > 0
    `).all();
