...
router.post('/sync/contacts', requireAuth, async (req, res) => {
  ...
  try {
    ...
    try { db.prepare('ALTER TABLE contacts ADD COLUMN google_contact_id TEXT').run(); } catch {}

    // schema: contacts.google_contact_id (canonical per current migrations)
    // v.170: was `google_id` — column is `google_contact_id`
    const upsert = db.prepare(`
      INSERT OR IGNORE INTO contacts
        (name, contact_type, email, phone_primary, phone_secondary, google_contact_id, company,
    ...
    for (const person of (data.connections || [])) {
      ...
      const existing = db.prepare('SELECT id FROM contacts WHERE google_contact_id=?').get(person.resourceName);
      if (!existing) {
