...
function getSnapshotData(memberId) {
  ...
  const kids = safe(() => db.prepare(
    `SELECT id, display_name, grade, allergies, school_name, notes FROM kids
  ...
  const perfumes = safe(() => db.prepare(
    `SELECT id, name, brand, status FROM perfumes
     WHERE family_member_id = ? ORDER BY status, name LIMIT 25`
  ).all(memberId));
  ...
  // books — schema audit shows no family_member_id, returns [] gracefully
  const books = safe(() => db.prepare(
    `SELECT id, title, author, status FROM books
     WHERE family_member_id = ?
     ORDER BY CASE status WHEN 'reading' THEN 0 WHEN 'queued' THEN 1 ELSE 2 END, title LIMIT 20`
  ).all(memberId));
