...
for p in index finance medical inventory property kids career books todos documents resources daily-log reports settings data notifications wardrobe perfume insurance subscriptions; do
...
# ── Wardrobe ─────────────────────────────────────────────────
section "Wardrobe"
assert_array "GET /wardrobe/items"    "$BASE/api/v1/wardrobe/items"
assert_array "GET /wardrobe/outfits"  "$BASE/api/v1/wardrobe/outfits"
assert_array "GET /wardrobe/planner"  "$BASE/api/v1/wardrobe/planner?start=2026-01-01&end=2026-12-31"
assert_keys  "GET /wardrobe/insights" "$BASE/api/v1/wardrobe/insights" "total"
...
code=$(fetch "$BASE/api/v1/trading/market/symbols")
if [ "$code" = "200" ] || [ "$code" = "400" ]; then
  if is_json; then pass "GET /trading/market/symbols  (HTTP $code, valid JSON)"
  else fail "GET /trading/market/symbols  (HTTP $code, not JSON)"; fi
else fail "GET /trading/market/symbols  (HTTP $code)"; fi
